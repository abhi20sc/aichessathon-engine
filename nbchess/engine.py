...
def allocate(time_left_ms: float, increment_ms: float, overhead_ms: float,
    ...
    hard = min(2.5 * soft, 0.5 * usable, usable - 1000.0)
...
class Engine:
    ...
    def __init__(self) -> None:
        ...
        self.killers = np.zeros((MAX_PLY, 4), dtype=np.uint32)
    ...
    def think(self, fen: str, budget_ms: float, hard_ms: float,
        ...
        for depth in range(1, max_depth + 1):
            ...
            if count > 0 and self.ctl[C_STOPPED] != 1:
                ...
                # An aborted iteration still finished some root moves, the
                # previous best first (the hash move leads the ordering) and
                # every one of them at the new depth with a full re-search
                # behind any score above the first. Prefer them to the
                # previous iteration: they are deeper, and the work is not
                # wasted, which is what makes starting the iteration safe.
                # Only a score above the pass's alpha is exact (or a lower
                # bound): a move that failed low carries an upper bound, which
                # says nothing about how much worse it might be.
                k = -count
                part = sorted(
                    ((move_to_uci(int(self.out_moves[i])), int(self.out_scores[i]))
                     for i in range(k) if int(self.out_scores[i]) > int(alpha)),
                    key=lambda p: p[1], reverse=True)
                if part:
                    seen = {p[0] for p in part}
                    ranked = part + [p for p in ranked if p[0] not in seen]
                    self.depth_reached = depth
