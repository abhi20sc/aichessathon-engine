...
CURVES = [
    ...
    ("shelter", I_SHELTER, N_SHELTER), ("storm", I_STORM, N_STORM),
]
SCALARS = [
    ...
    ("threat mg", I_THREAT_MG), ("threat eg", I_THREAT_EG),
    ("rook open mg", I_ROPEN_MG), ("rook open eg", I_ROPEN_EG),
    ("rook semi mg", I_RSEMI_MG), ("rook semi eg", I_RSEMI_EG),
    ("passed king enemy", I_PK_ENEMY), ("passed king own", I_PK_OWN),
    ("pawn mg", I_MAT_MG), ("knight mg", I_MAT_MG + 1), ("bishop mg", I_MAT_MG + 2),
    ("rook mg", I_MAT_MG + 3), ("queen mg", I_MAT_MG + 4),
    ("pawn eg", I_MAT_EG), ("knight eg", I_MAT_EG + 1), ("bishop eg", I_MAT_EG + 2),
    ("rook eg", I_MAT_EG + 3), ("queen eg", I_MAT_EG + 4),
]
...
def main() -> None:
    ...
    ap.add_argument("--py", type=Path, default=None,
                    help="also write the weights as a Python module (nbchess/tuned.py)")
    args = ap.parse_args()
    ...
    if args.py is not None:
        rows = [", ".join(str(int(x)) for x in w[i:i + 12]) for i in range(0, len(w), 12)]
        head = ('"' * 3) + "Generated by tools.tune: do not edit. See terms.py for what each weight is." + ('"' * 3)
        args.py.write_text(
            head + "\n"
            f"# {states.shape[0]:,} positions from {n_games:,} games, K = {best_k:.2f}, "
            f"holdout error {start_val:.6f} -> {final_val:.6f}\n"
            "WEIGHTS = [\n" + "".join(f"    {r},\n" for r in rows) + "]\n")
        print(f"wrote {args.py}")
